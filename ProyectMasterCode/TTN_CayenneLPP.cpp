...
uint8_t TTN_CayenneLPP::addTIME(uint8_t channel, uint32_t time){
    if((cursor + LPP_TIME_SIZE) > maxsize){
        return 0;
    }
    // buffer[cursor++] = channel;
    // buffer[cursor++] = LPP_TIME;
    buffer[cursor++] = time >> 24;
    buffer[cursor++] = time >> 16;
    buffer[cursor++] = time >> 8;
    buffer[cursor++] = time;
    return cursor;
}